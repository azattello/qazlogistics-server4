...
router.put('/updateFilial/:id', async (req, res) => {
  try {
      const { id } = req.params;
      const { filialText, userPhone, filialId, filialAddress } = req.body;

      // Проверка, существует ли пользователь с указанным номером телефона
      const newUser = await User.findOne({ phone: userPhone });
      if (!newUser) {
          return res.status(400).json({ message: 'Пользователь с указанным номером телефона не найден' });
      }

      // Найти текущий филиал по ID
      const filial = await Filial.findById(id);
      if (!filial) {
          return res.status(404).json({ message: 'Филиал не найден' });
      }

      // Проверка, если пользователь уже владеет филиалом, вернуть ошибку
      if (newUser.role === 'filial' && newUser._id.toString() !== filial.userId.toString()) {
          return res.status(400).json({ message: 'Пользователь уже владеет другим филиалом' });
      }

      // Обновить роль нового пользователя на "filial"
      newUser.role = 'filial';
      await newUser.save();

      // Найти предыдущего владельца филиала
      const oldUser = await User.findById(filial.userId);
      if (oldUser) {
          // Обновить роль старого пользователя на "client"
          oldUser.role = 'client';
          await oldUser.save();
      }

      // Обновить данные филиала и связать его с новым пользователем
      filial.filialText = filialText;
      filial.userPhone = userPhone;
      filial.filialId = filialId;
      filial.filialAddress = filialAddress;
      filial.userId = newUser._id; // Связь с новым пользователем
